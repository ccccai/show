...
<hr><br>
<div id="line_user_duration"></div><br>
<div id="line_user_avg_duration"></div>
</body>
<script>
    //日期选择框配置
...
//                        console.log(oRsp);
                        oRspJson = JSON.parse(oRsp);
                        if (oRspJson.success) {
                            console.log(oRspJson.appOpenDuration);
                            aoYUserDuration = [];
                            aoYUserAvgDuration = [];
                            var k = 0;
                            for (var key in oRspJson.appOpenDuration) {
                                aoYUserDuration.push({name: key, data: []});
                                aoYUserAvgDuration.push({name: key, data: []});
                                for (var i = 0; i < 17; i++) {
                                    aoYUserDuration[k].data.push(parseInt((oRspJson.appOpenDuration[key][i] / 60).toFixed(0)));
                                    aoYUserAvgDuration[k].data.push(parseInt((oRspJson.appOpenDuration[key][i] / datesLength / 60).toFixed(0)));
                                }
                                k++;
                            }
                            funUserDurationsLine('line_user_duration', '各应用各时间段总使用时长折线图', aoYUserDuration);
                            funUserAvgDurationsLine('line_user_avg_duration', '各应用各时间段平均使用时长折线图', aoYUserAvgDuration);
                        }
                        else
        ...
        funcQueryTree();
        //定义绘图函数
        var funUserDurationsLine = function (strDomId, strTitle, paoY) {
        ...
        var funUserAvgDurationsLine = function (strDomId, strTitle, paoY) {
            Highcharts.chart(strDomId, {
                title: {
                    text: strTitle,
                },
