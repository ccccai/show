	...
	public function read_record($offset){
		...
		if ($flag == 1){
			...
			if ($sub_flag == 2){
				...
				$record[0] = mb_convert_encoding($this->read_location($country_offset),'UTF-8','GBK');
				// 地区
				$record[1] = mb_convert_encoding($this->read_location($location_offset+4),'UTF-8','GBK');
			} else {
				$record[0] = mb_convert_encoding($this->read_location($location_offset),'UTF-8','GBK');
				$record[1] = mb_convert_encoding($this->read_location($location_offset+strlen($record[0])+1),'UTF-8','GBK');
			}
		} else if ($flag == 2){
			// 地区
			// offset + 1(flag) + 3(country offset)
			$record[1] = mb_convert_encoding($this->read_location($offset+4),'UTF-8','GBK');
			...
			$record[0] = mb_convert_encoding($this->read_location($offset),'UTF-8','GBK');
			$record[1] = mb_convert_encoding($this->read_location($offset+strlen($record[0])+1),'UTF-8','GBK');
		}
		return $record;
	}
